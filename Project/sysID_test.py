...
def sysIDed_TF(m, n, data):
    ...
    # nysquist frequency 
    nf = sf / 2
    print("Sampling frequency is", sf)
    print("Nyquist frequency is", nf)
    ...
    # so now we have all the inputs (up to N), but we want to then truncate the input 
    # up to the datapoint corresponding to the Nyquist frequency 

    f = fft.rfftfreq(N, d=dt)  # the frequencies in Hz, we define these
    # The frequencies are [ 0.    0.01  0.02 ...  9.98  9.99 10.  ] so 1000 total

    # N_f_max = np.searchsorted(f, chirp_freq_high)  # find index of max frequency (chirp higher bound)
    N_f_max = np.searchsorted(f, nf)  # find index of max frequency (Nyquist frequency) 
    print("N_f_max is", N_f_max)
    ...
    # Define frequency limits in rad/s
    lower_chirp_freq_rad = chirp_freq_low * 2 * np.pi  # convert Hz to rad/s
    upper_limit_rad = 8 # Hz
    upper_limit_Hz = upper_limit_rad 

    # Mask for frequency range between lower chirp and 8 rad/s, below Nyquist
    mask = (f >= chirp_freq_low) & (f <= upper_limit_Hz)
    f = f[mask]
    omega = f * 2 * np.pi
    N_f_max = len(f)
    ...
    # now define the max frequency we want to include to EXCLUDE the 
    # datapoints that are too close to the Nyquist frequency 
    ...
    axes[0].set_yticks(np.arange(-20, 1, 5)) 
    axes[0].set_ylim(-20, 0)  # y-axis from -20 to 0 dB
    ...
    axes[1].set_yticks(np.arange(0, 91, 15)) 
    axes[1].set_ylim(0, -90)  # y-axis from 0 to -90 degrees
...
G = sysIDed_TF(0, 1, data=data_read)
print(G)

# note to self 
# what we want is to shrink the data up to the nyquist frequency.
# dft gives us the coefficients of the modes to find the dominant mode
# with this, we know which modes are dominant in our signal 
# the nyquist frqueny is half the sampling frequency 
# so withthe nyquist frquency, we can truncate the data to 
# remove the datapoints which contribute to frequencies higher
# than the Nyquist frequncy 
