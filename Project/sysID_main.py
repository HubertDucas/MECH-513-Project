...
def averaged_training_CSD(combs, data):
    ...
    for comb in combs:
        ...
        for d in comb:
            ...
            # Puy and Puu (normalized)
            f, Puy = signal.csd(u_nor, y_nor, fs=1/dt, window='hann')
            f, Puu = signal.csd(u_nor, u_nor, fs=1/dt, window='hann')
            ...
            # un-normalize each 
            # Gd = (u_max / y_max) * (Puy / Puu)
            Gd = (Puy / Puu)
            
            for i in range(len(f)):
                P_holder[i, l] = Gd[i] #Puy[i] / Puu[i] # CSD_d
            l += 1
    ...
    f_min = 0.1 # lower chirp frequency 
    f_max = 8 # right before it blows up 
    idx = f <= f_max
    f = f[idx]
    CSD_mat = CSD_mat[idx, :]
...
#     # Get frequency array from first dataset
#     u_raw = data_read[:, 1]
#     y_raw = data_read[:, 2]
#     f, _ = signal.csd(u_raw, y_raw, fs=1/dt, window='hann')
...
#         # Store Puy and Puu for each dataset in this combination
#         Puy_holder = np.zeros((len(f), no_datasets), dtype=complex)
#         Puu_holder = np.zeros((len(f), no_datasets), dtype=complex)
        
#         # Loop through each dataset of this combination
#         for l, d in enumerate(comb):
#             training_dataset_d = data[d, :, :]
#             u = training_dataset_d[:, 1].ravel()
#             y = training_dataset_d[:, 2].ravel()
            
#             # Compute CSDs WITHOUT normalization
#             f, Puy = signal.csd(u, y, fs=1/dt, window='hann')
#             f, Puu = signal.csd(u, u, fs=1/dt, window='hann')
...
#         # Average Puy and Puu separately across datasets
#         Puy_avg = np.mean(Puy_holder, axis=1)
#         Puu_avg = np.mean(Puu_holder, axis=1)
        
#         # THEN take the ratio
#         CSD_mat[:, k] = Puy_avg / Puu_avg
...
def LS_fit(m, n, CSD_mat, f):
    ...
    for col in range(N_comb):
        ...
        # b_vals = (omega ** 2) * G_col
        b_vals = -G_col * (1j * omega) ** n
        print("b matrix", b_vals)  
        ...
        for k in range(m + 1):
            for i_freq in range(N_w):
                n_mat[i_freq, k] = -(1j * omega[i_freq])**k  
                # n_mat[i_freq, k] = (1j * omega[i_freq])**k 
        ...
        for l in range(n):
            for i_freq in range(N_w):
                # d_mat[i_freq, l] = G_col[i_freq] * (1j * omega[i_freq])**l
                d_mat[i_freq, l] = G_col[i_freq] * (1j * omega[i_freq])**l
        ...
        # Construct transfer function for this combination
        num = x[:m + 1].flatten()[::-1]
        den = np.concatenate(([1], x[m + 1:].flatten()[::-1]))
        print("x solution:", x.flatten())
        print("num coeffs:", num)
        print("den coeffs:", den)
        # After getting TFs[0], pick a frequency index, say i=5
        omega_test = omega[5]
        s_test = 1j * omega_test

        # Manual evaluation (for m=0, n=1):
        G_manual = num[0] / (s_test + den[1])

        # What you got from CSD:
        G_measured = CSD_mat[5, 0]

        print(f"G_manual: {G_manual}")
        print(f"G_measured: {G_measured}")
        print(f"Ratio (dB): {20*np.log10(abs(G_measured/G_manual))}")
        G_est = control.tf(num, den)
...
def plotting(f, CSD_mat, TFs):
    ...
    for i in range(N_comb):
        ...
        # --- Experimental (averaged) frequency response ---
        mag_exp = 20 * np.log10(np.abs(G_exp))
        phase_exp = np.unwrap(np.angle(G_exp, deg=True))

        # --- Fitted transfer function response ---
        mag_fit, phase_fit, omega_fit = control.frequency_response(TFs[i], omega)
        mag_fit = np.squeeze(mag_fit)
        phase_fit = np.squeeze(phase_fit)
        mag_fit_dB = 20 * np.log10(mag_fit)
        phase_fit_deg = np.unwrap(np.degrees(phase_fit))

        # --- Magnitude plot ---
        axs[0, i].semilogx(f, mag_exp, 'b', label='Measured FRF')
        axs[0, i].semilogx(f, mag_fit_dB, 'r--', label='LS Fit')
        ...
        # --- Phase plot ---
        axs[1, i].semilogx(f, phase_exp, 'b', label='Measured FRF')
        axs[1, i].semilogx(f, phase_fit_deg, 'r--', label='LS Fit')
